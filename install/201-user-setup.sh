...
configure_sudo_family() {
    ...
    # Create user-specific sudoers file in sudoers.d (safer than editing main sudoers)
    print_info "Creating user sudo configuration in sudoers.d..."
    
    cat > "$user_sudoers_file" << EOF
# ALIE - Sudo configuration for user: $USERNAME
# Generated by: ALIE 201-user-setup.sh
# Tool: $PRIV_TOOL (${config_type})
# 
# This file provides sudo access even when doas is the primary tool
# for maximum compatibility with packages that hard-require sudo

# Allow $USERNAME to run all commands with password
$USERNAME ALL=(ALL:ALL) ALL

# Allow wheel group access (standard configuration)
%wheel ALL=(ALL:ALL) ALL

# Optional: Passwordless commands for common system operations
# Uncomment and modify as needed for your security requirements:
# $USERNAME ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart NetworkManager
...
EOF
    ...
    # Add useful sudo configurations to sudoers.d
    local sudo_config_file="$sudoers_dir/00-alie-defaults"
    
    # Only create global defaults if this is the primary tool
    if [ "$config_type" = "primary" ]; then
        cat > "$sudo_config_file" << 'EOF'
# ALIE - Global sudo defaults
# More secure and user-friendly sudo settings

# Keep environment variables that are useful
Defaults env_keep += "HOME LANG LC_* TZ EDITOR PAGER"

# For GUI applications when using sudo
Defaults env_keep += "DISPLAY XAUTHORITY"

# Extend timeout to 15 minutes (default is 5)
Defaults timestamp_timeout=15

# Show asterisks when typing password
Defaults pwfeedback

# Don't lecture users (suppress the "lecture" on first sudo use)
Defaults !lecture

# Use a separate log file for sudo commands (security)
Defaults logfile=/var/log/sudo.log

# Require a tty for sudo (security - prevents some attacks)
Defaults requiretty
EOF
    else
        # For backup configuration, use minimal settings
        cat > "$sudo_config_file" << 'EOF'
# ALIE - Minimal sudo defaults (backup configuration for doas compatibility)
# This provides basic sudo functionality when doas is the primary tool

# Keep essential environment variables
Defaults env_keep += "HOME LANG LC_* TZ DISPLAY XAUTHORITY"

# Don't lecture users
Defaults !lecture

# Log sudo usage for security auditing  
Defaults logfile=/var/log/sudo.log
EOF
    fi
    ...
    if visudo -c &>/dev/null; then
        print_success "✓ $PRIV_TOOL configured with sudoers.d approach"
...
# Configure doas according to ArchWiki best practices
configure_doas() {
    local doas_conf="/etc/doas.conf"
    
    print_info "Configuring OpenDoas according to ArchWiki standards..."
    
    # Create precise doas configuration following ArchWiki
    cat > "$doas_conf" << EOF
...
EOF
    ...
    if doas -C "$doas_conf" &>/dev/null; then
        print_success "✓ doas configuration syntax is valid"
    else
    ...
    print_success "OpenDoas configured successfully"
...
EOF
    ...
    if timeout 5 run0 --dry-run true &>/dev/null; then
        print_success "✓ run0 is working correctly"
    else
