...
show_manual_menu() {
    ...
    echo ""
    echo "  ${CYAN}1)${NC} Disk Partitioning (001-base-install.sh) - Partition/format disks (Live USB, root)"
    echo "  ${CYAN}2)${NC} Shell/Editor Selection (002-shell-editor-select.sh) - Select shells/editors (Live USB, root, optional)"
    echo "  ${CYAN}3)${NC} System Installation (003-system-install.sh) - Install base system with pacstrap (Partitioned, root)"
    echo "  ${CYAN}4)${NC} System Configuration (101-configure-system.sh) - Configure timezone/locale/hostname/GRUB (Chroot, root)"
    echo "  ${CYAN}5)${NC} User Setup (201-user-setup.sh) - Create desktop user, sudo/doas, basic tools (Booted system, root)"
    echo "  ${CYAN}6)${NC} AUR Helper Installation (211-install-aur-helper.sh) - Install yay/paru + optimize makepkg (Booted, user)"
    echo "  ${CYAN}7)${NC} CLI Tools Selection (212-cli-tools.sh) - Interactive CLI tools/utilities selection (AUR helper, user)"
    echo "  ${CYAN}8)${NC} Display Server Setup (213-display-server.sh) - Choose Xorg/Wayland/Both (Booted, root)"
    echo "  ${CYAN}9)${NC} Desktop Selection (220-desktop-select.sh) - Choose DE/WM/Skip (Display server, root)"
    echo "  ${CYAN}A)${NC} Desktop Environment (221-desktop-environment.sh) - Install Cinnamon/GNOME/KDE/XFCE4 (Desktop selected, root)"
    echo "  ${CYAN}B)${NC} X11 Window Manager (222-window-manager.sh) - Install i3/bspwm/Openbox/etc (Desktop selected, root)"
    echo "  ${CYAN}C)${NC} Wayland Window Manager (223-wayland-wm.sh) - Install Sway/Hyprland/etc (Desktop selected, Wayland, root)"
    echo "  ${CYAN}D)${NC} Desktop Tools (231-desktop-tools.sh) - Install LibreOffice/GIMP/Firefox/etc (DE/WM installed, root)"
    echo ""
